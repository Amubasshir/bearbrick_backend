...
- ~~**Canonical brick image field stores an expiring 1-hour signed URL.** `approveAndApply` copies the submission's `content_url` (a 1-hour signed URL) verbatim into the canonical brick image column, so the stored reference expires.~~ **RESOLVED post-Phase-B (goodwill item 2):** `approveAndApply` now stores a **durable bucket object path** (bare key `{userId}/{uuid}.{ext}`, captured on the new additive `content_path` column — migration `20260902000100`) in the canonical image field, and `src/lib/brickImages.js#signBrickImageFields` re-signs it into a fresh time-limited URL on serve (wired into `AdminBrickController.create/update`). See the goodwill-batch note below.
- ~~**`mark_paid` allows `REQUESTED → PAID` directly** (bypassing `approve`) — Phase A PayoutService behavior, characterized by a test and flagged.~~ **RESOLVED post-Phase-B (goodwill item 1):** `PayoutService.markPaid` now requires the payout be `APPROVED`; a `mark_paid` on a `REQUESTED` payout returns `cannot_pay_requested` (→ 409). Requested → Approved → Paid is enforced (Rejected as the alternate path); the idempotent double-mark-paid no-op is preserved. See the goodwill-batch note below.
...
### Post-Phase-B Goodwill Batch (Shipped)

Three client-requested post-delivery changes, all shipped after Phase B (strict TDD, additive except the two sanctioned service changes noted). Clean-DB baseline grew **716 → 728**.

**Item 1 — payout workflow (sanctioned):** `PayoutService.markPaid` now requires the payout be in `APPROVED` state (single-line guard: non-`APPROVED` → `cannot_pay_' + status`). A `mark_paid` on a `REQUESTED` payout returns `cannot_pay_requested` → **409** (via the existing 3.10 `cannot_*` mapping — no controller change). **Requested → Approved → Paid** is enforced, with **Rejected** as the alternate path from REQUESTED/APPROVED. `cannot_pay_rejected` and the `paid_at IS NULL` idempotent double-mark-paid no-op are preserved. approve / reject / reserve accounting untouched. The Phase-B `REQUESTED→PAID` characterization test was flipped to assert the 409, and the Q10 multi-payout test now `approve`s before `markPaid`; +1 new service guard test.

**Item 2 — durable official-image reference (sanctioned):** Approved contributor photos now store a **permanent bucket object path** (bare key `{userId}/{uuid}.{ext}`) in the canonical brick image field instead of an expiring signed URL. Two-part fix, since the durable path was not persisted anywhere before: **(a) capture** — additive migration `20260902000100_bounty_submissions_content_path` adds nullable `content_path` (+ Prisma field); `UploadController` now returns `contentPath` alongside `contentUrl`; `BountyController.createSubmission` threads it; `submit()` persists it. **(b) store** — `applyToBrick` writes `content_path` for IMAGE bounties (falls back to `content_url` for pre-change submissions; DATA path unchanged). **(c) serve** — new `src/lib/brickImages.js#signBrickImageFields(brick,{ttl,storage})` re-signs the five bounty image fields that hold a bare path (skips null / already-absolute URLs; touches storage only when a signable field exists, so hermetic tests never reach it), wired into `AdminBrickController.create/update`. A refreshed live smoke (`scripts/smoke-bounty-upload.js`) confirmed end-to-end: canonical field = durable path, and a fresh URL re-signed from the stored path resolves HTTP 200. +11 additive tests (helper unit + controller sign-on-serve + capture + durable-apply).

**Item 3 — reward configurability (verify-only, no code):** confirmed already satisfied — per-type **cash and credit reward amounts are seeded DB rows** (`bounty_definitions`, captured on the submission at submit time) and are **admin-editable via `PATCH /api/admin/bounty-definitions/:id`** (`reward_cash_cents`, `reward_credits`, `priority`, `is_active`; flag `can_manage_settings`). XP is **derived from `priority`** via the `XP_BY_PRIORITY` code map (no `xp` column), so it is tunable only indirectly via the priority tier — noted below.

**Still open after the goodwill batch (for the next milestone):**

- **XP is not a directly-settable admin number.** It follows the priority tier via `calculateBountyXp`/`XP_BY_PRIORITY` (HIGH 15 / MEDIUM 10 / LOW 5); the milestone bonuses (25/100/50) are code constants too. Making XP first-class admin-editable would mean a nullable `reward_xp` column on `bounty_definitions` (+ the PATCH whitelist + `calculateBountyXp` preferring it) — fold into the challenges milestone if wanted. Amounts remain placeholders pending client confirmation.
- **`AdminDashboardController` brick reads are not yet sign-on-serve.** `listBricks` / `featureBrick` return the raw brick record (now a durable path in the bounty image fields); they were deliberately **not** wired (both are under the documented flaky `dex-admin-dashboard.test.js` and are admin management, not image display). Adopt `signBrickImageFields` there when the user-facing image-display layer is built (frontend milestone).
- **Personal contribution challenges (item 4)** are the agreed next-milestone build — this is the "contribution activation" unit noted in the deferrals above (bounty → challenge-progress bridge + a real leaderboard weight query), not a config flip.
- **Public leaderboards remain out** by client choice.
